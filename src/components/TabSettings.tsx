...
export function TabSettings() {
  ...
  const handleBulkDelete = (type: 'elements' | 'templates') => {
  ...
  const toggleAll = (type: 'elements' | 'templates') => {
    if (type === 'elements') {
      if (selectedElements.length === store.state.elements.length) {
        setSelectedElements([]);
      } else {
        setSelectedElements(store.state.elements.map(e => e.id));
      }
      setConfirmingElements(false);
    } else {
      if (selectedTemplates.length === store.state.templates.length) {
        setSelectedTemplates([]);
      } else {
        setSelectedTemplates(store.state.templates.map(t => t.id));
      }
      setConfirmingTemplates(false);
    }
  };
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex flex-col shadow-sm max-h-[600px] rounded-xl overflow-hidden">
                ...
                <div className="flex items-center gap-2">
                  {confirmingElements && (
                    <button 
                      ...
                      className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold text-xs hover:bg-slate-200 dark:hover:bg-slate-700 transition uppercase tracking-widest"
                    >
                      Cancel
                    </button>
                  )}
                  <button
                    ...
                    className={`px-3 py-1.5 font-bold text-xs border disabled:opacity-50 transition flex items-center gap-1 uppercase tracking-widest ${
                      confirmingElements 
                  ...
                  >
                    {confirmingElements ? <AlertTriangle className="w-3.5 h-3.5" /> : <Trash2 className="w-3.5 h-3.5" />}
              ...
              <div className="p-2 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                <button onClick={() => toggleAll('elements')} className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest px-2 py-1 hover:bg-indigo-50 dark:hover:bg-indigo-900/40 transition">
                  Toggle All
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1 bg-white dark:bg-slate-900 custom-scrollbar">
                {store.state.elements.length === 0 ? (
                  ...
                  store.state.elements.map(el => (
                    <label key={el.id} className="flex items-center gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition">
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4 text-indigo-600 border-slate-300 dark:border-slate-600 rounded focus:ring-indigo-500 bg-white dark:bg-slate-800"
                      />
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex flex-col shadow-sm max-h-[600px] rounded-xl overflow-hidden">
              <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {confirmingTemplates ? <AlertTriangle className="w-3.5 h-3.5" /> : <Trash2 className="w-3.5 h-3.5" />}
              ...
              <div className="p-2 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                <button onClick={() => toggleAll('templates')} className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest px-2 py-1 hover:bg-indigo-50 dark:hover:bg-indigo-900/40 transition">
                  Toggle All
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1 bg-white dark:bg-slate-900 custom-scrollbar">
                {store.state.templates.length === 0 ? (
                  ...
                  store.state.templates.map(t => (
                    <label key={t.id} className="flex items-center gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition">
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4 text-indigo-600 border-slate-300 dark:border-slate-600 rounded focus:ring-indigo-500 bg-white dark:bg-slate-800"
                      />
                      <span className="font-bold text-slate-800 dark:text-slate-100 text-sm truncate">{t.name}</span>
                    </label>
                  ))
                )}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { label: 'Categories', data: uniqueCategories, field: 'category' as const, color: 'text-amber-600 bg-amber-50 dark:bg-amber-900/20' },
                { label: 'Types', data: uniqueTypes, field: 'type' as const, color: 'text-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' },
                { label: 'Companies', data: uniqueCompanies, field: 'company' as const, color: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20' },
              ].map(section => (
                <div key={section.label} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex flex-col shadow-sm rounded-xl overflow-hidden max-h-[300px]">
                  ...
                  <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                    {section.data.length === 0 ? (
                      ...
                      section.data.map(val => (
                        <div key={val} className="flex items-center justify-between gap-2 p-2 group hover:bg-slate-50 dark:hover:bg-slate-800 border border-transparent hover:border-slate-100 dark:hover:border-slate-700 transition">
                          <span className={`px-2 py-0.5 rounded text-[11px] font-black uppercase tracking-tight truncate ${section.color}`}>
                            {val}
                          </span>
                          <button
                            onClick={() => {
                              if (confirm(`Clear all instances of "${val}" from elements?`)) {
                                store.deleteMetadata(section.field, val);
                              }
                            }}
                            className="p-1 px-2 text-red-500 opacity-0 group-hover:opacity-100 transition hover:bg-red-50 dark:hover:bg-red-900/30 rounded"
                            title={`Delete all ${val}`}
                          >
